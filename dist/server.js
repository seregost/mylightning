...
var LevelStoreCreator = require("express-session-level");
...
var v1_wallet_1 = require("./routes/v1-wallet");
...
var wallet = new v1_wallet_1.WalletService();
...
Passport.use(new PassportLocal.Strategy(function (username, password, done) {
    process.nextTick(function () {
        ...
        if (!user) {
            ...
            return userManager.UpdatePassword(username, password, function (user) {
                if (user == null) {
            ...
            return userManager.VerifyPassword(username, password, function (success, user) {
                if (success == true) {
...
app.use('*', function (req, res, next) {
    ...
    if (req.isAuthenticated()) {
        if (usermanager_1.UserManager.getInstance().GetUser(req.user.id).rpcport == null) {
...
var wss = new WebSocket.Server({
    verifyClient: function (info, done) {
        var response = {};
        sessionParser(info.req, response, function () {
