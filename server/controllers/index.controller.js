...
(function() {
  ...
  .controller('MainController', ['$scope', 'lightningService', function($scope, lightningService) {
    var vm = this;
      ...
      if(amount > 0) {
        ...
        lightningService.execQuickPay(dest, amount, memo).then((response) => {
          if(response.error == null) {
            vm.quickpay.memo = "";
            vm.quickpay.amount = 0;
            vm.quickpay.haserror = false;
            angular.element('#quickpaymodal').modal('toggle');
          }
          else {
